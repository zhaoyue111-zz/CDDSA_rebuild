...
class FundusDataset(Dataset):
    '''
        获取一个特定域的所有图片、标签、域号
    '''
    def __init__(self, root_dir="./OD", domain=0, mode='train', transform=None):
        ...
        # 获取图像和分割标签路径
        self.image_dir = os.path.join(self.root_dir, mode, 'image')
        self.mask_dir = os.path.join(self.root_dir, mode, 'mask')
...
class MultiDomainIterator:
    def __init__(self, root_dir, domain_list, sample_list, mode='train', transform=None):
        '''
        domain_list:[0,2,3]
        sample_list:[3,4,5]
        mode:train/val
        ...
        self.datasets = {
            domain: FundusDataset(root_dir, domain, mode=mode, transform=transform)
...
if __name__ == '__main__':
    ...
    iterator = MultiDomainIterator(root_dir, domain_list, sample_list,mode='train')
